...
class APIFeatures<T> {
  ...
  private queryString: QueryParams<T>;

  constructor(query: FilterQuery<T>, queryString: QueryParams<T>) {
    this.query = query;
  ...
  paginate() {
    ...
    const limit = this.queryString.limit ? Number(this.queryString.limit) : 100;
